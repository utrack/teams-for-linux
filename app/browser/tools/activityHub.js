	...
	setUserStatus(status) {
		...
		if (teams2IdleTracker) {
			...
			instance
				.whenReady()
				.then((inst) => {
					inst.controller.presenceService.setMyStatus(status, null, true);
				})
				.catch(() => {
...
function performPlatformTweaks(controller) {
	...
	controller.callingService.callingAlertsService.isRunningOnWindows = () =>
		isRunningOnWindows;
}
...
function getMeetingEvents(data) {
	return data.filter((d) => {
		return (
			d.messagetype === "Event/Call" &&
			d.content === '<partlist alt =""></partlist>'
		);
	});
}
...
function addEligibleCalendarEvents(
	calendarEvents,
	meetingId,
	notificationList,
) {
...
function assignActivitiesCountUpdateHandler(controller) {
	// TODO utrack
	// controller.eventingService.asObservable.subscribe(
	// 	controller.$scope,
	// 	controller.constants.events.notifications.bellCountUpdated,
	// 	() => onActivitiesCountUpdated(controller));

	controller.chatServiceClient.conversationsAsyncService.subscribe(() =>
		onActivitiesCountUpdated(controller),
	);
...
function assignIncomingCallCreatedHandler(controller) {
	controller.eventingService.asObservable.subscribe((e, data) => {
		if (data.signalingSession.isIncomingCall) {
			onIncomingCallCreated({
				caller: data.signalingSession.remoteCaller.displayName,
			});
		}
	}, "call_created");
}
...
async function onMyStatusChanged(controller, data) {
	...
	for (const handler of handlers) {
		handler.handler({
			data: data,
			isInactive:
				window.teamspace.services.ApplicationState[
					controller.appStateService.current
				] === "Inactive" ||
				window.teamspace.services.ApplicationState[
					controller.appStateService.current
				] === "LongInactive",
