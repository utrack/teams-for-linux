...
async function overrideMSMethod(inst) {
	bgMSService = inst.controller.customVideoBackgroundsService;
...
async function customBGProvider(...args) {
	const ms_response = config.customBGServiceIgnoreMSDefaults
		? []
		: await bgMSMethod.apply(bgMSService, [...args]);
